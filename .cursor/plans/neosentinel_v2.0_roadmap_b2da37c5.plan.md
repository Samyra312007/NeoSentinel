...
**Context:** Greenfield repo at [`C:\Users\kumar\OneDrive\Desktop\NeoSentinel`](C:\Users\kumar\OneDrive\Desktop\NeoSentinel) (0 files today). Target: production-grade demo on **3× AWS Graviton4 (c8g.4xlarge)** with **tests at every step**.
...
1. **Repo scaffold**
   - [`pyproject.toml`](pyproject.toml) — Python 3.12, package name `neosentinel`, dev deps (`pytest`, `hypothesis`, `ruff`)
   - [`LICENSE`](LICENSE) — Apache 2.0
   - [`.github/workflows/ci.yml`](.github/workflows/ci.yml) — lint + test skeleton (Divyansh leads, Sahil reviews)
   - Directory layout per spec (empty `__init__.py` files OK)

2. **Frozen contracts** in [`neosentinel/contracts/`](neosentinel/contracts/) (both sign off):
   - `telemetry.py` — `TelemetrySnapshot`, `NodeSnapshot`, `BaselineMetrics`
   - `decision.py` — `SentinelDecision`, `ActionType` (7 enum values)
   - `streams.py` — Redis stream names, field schemas for all 4 streams
   - `websocket.py` — dashboard event types (`metrics`, `agent_thought`, `healing`, `audit`, `flame_graph`)
   - `openapi.yaml` — dashboard REST + WS endpoints

3. **3-node infra provisioning doc** (Sahil writes, both use):
   - AWS EC2 checklist: 3× c8g.4xlarge, Ubuntu 24.04 ARM64, security group ports (22, 8000, 8080, 6379, 7000, 10001)
   - SSH key setup, node naming (`node-001`, `node-002`, `node-003`)

**Exit criteria:** CI runs green on empty package; contract files merged; 3 Graviton4 instances reachable via SSH.
